...
def get_device(path, usb_ids, debounce_seconds, token_id_handler):
    ...
    if (device.info.vendor, device.info.product) not in usb_ids:
        return None
    return Device(ev_device = device, debounce_seconds = debounce_seconds, token_id_handler = token_id_handler)
...
def main():
    ...
    all_devices = [get_device(path, usb_ids, debounce_seconds, token_id_handler) for path in evdev.util.list_devices()]
